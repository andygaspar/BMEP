...
public:
	...
	Solver(){}
	Solver(int CoreNum,int taxa){
		XPRSsetintcontrol(problem.getXPRSprob(), XPRS_OUTPUTLOG, 0);
		XPRSsetintcontrol(problem.getXPRSprob(), XPRS_LPLOG, 0);
		XPRSsetintcontrol(problem.getXPRSprob(), XPRS_MIPLOG, 0);
		Core=CoreNum; 
		n=taxa;
...
//	XPRSsetintcontrol(problem.getXPRSprob(), XPRS_OUTPUTLOG, 0);
	...
	for(register unsigned int i=1; i<=n-1; i++){
		for(register unsigned int j=i+1; j<=n; j++){
			for(register unsigned int k=2; k<=n-1; k++){
				double coeff; 
				if(VinhHaeseler) coeff =  2*dist[i][j]/precomputedpowVH[k];           //pow(2.0,(double)k/n);
...
bool Solver::Pruned(int taxon, int selector){
	double LowerBound=0; 
	switch(selector){
		case 1: LowerBound=PardiLowerBound(taxon); if(LowerBound > Optimum.tree_len) return true; break;
		case 2: if(!VinhHaeseler){
...
void Solver::DFS(int taxon){ 
	...
	if(taxon==n){
	    ...
	    return;                   
	   	...
	   	if(Pruned(taxon,boundtype)==true) return;	
		  	else{
...
void Solver::SetNode_and_Run(NODE *node){
	...
	if(FastLinearProgrammingLowerBound(taxon) < Optimum.tree_len)
    //Explore	
