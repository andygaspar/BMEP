...
class ALPHA(nn.Module):
    ...
    def forward(self, hi, hj, d, d_mask):
        a = self.f_alpha(torch.cat([hi, hj], dim=-1))
        a = self.v(a).view(d.shape)
        alpha = F.softmax(torch.mul(a, d) - 9e15 * (1 - d_mask), dim=-2)
        return alpha
...
class DGN_5(Network):
    def __init__(self, num_inputs, att_inputs, hidden_dim, network=None):
        ...
        self.d_mat_encoder = nn.Sequential(
    ...
    def forward(self, adj_mats, d_mats, initial_masks, masks):
        ...
        h = self.message_round(h, d_mats, d_mask, self.rounds)
    ...
    def message_round(self, h, d, d_mask, rounds):

        for i in range(rounds):
            hi, hj = self.i_j(h, d.shape)
            alpha = self.alpha[i](hi, hj, d, d_mask).unsqueeze(-1)
