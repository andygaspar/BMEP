...
class MessageSeparated(nn.Module):
    def __init__(self, h_dimension, hidden_dim, device):
        self.device = device
...
class MessageNodeSeparated(nn.Module):
    def __init__(self, h_dimension, hidden_dim, device):
        self.device = device
        super(MessageNodeSeparated, self).__init__()
        self.fmn_leafs = nn.Linear(h_dimension + hidden_dim, h_dimension).to(self.device)
        self.fmn_internal = nn.Linear(h_dimension + hidden_dim, h_dimension).to(self.device)

    def forward(self, h, m1, leafs_mask, internal_mask):
        leafs = torch.tanh(self.fmn_leafs(torch.cat([h, m1], dim=-1))) * leafs_mask
        internals = torch.tanh(self.fmn_internal(torch.cat([h, m1], dim=-1))) * internal_mask
        out = leafs + internals
        return out
...
class GNN_2(Network):
    def __init__(self, net_params, network=None):
        ...
        self.alpha_adj = nn.ModuleList([MessageSeparated(h_dimension, hidden_dim, self.device) for _ in range(self.rounds)])
        self.alpha_all = nn.ModuleList([MessageSeparated(h_dimension, hidden_dim, self.device) for _ in range(self.rounds)])

        self.fm1 = MessageNode(h_dimension, hidden_dim, self.device)
        self.fm2 = MessageNodeSeparated(h_dimension, hidden_dim, self.device)
        self.fm3 = MessageNodeSeparated(h_dimension, hidden_dim, self.device)
    ...
    def message_round(self, h, d, d_mask, adj_mats, size_masks, initial_mask, ad_masks, rounds):
        ...
        for i in range(rounds):
            ...
            leafs_mask = initial_mask[:, :, 0].unsqueeze(-1).expand(-1, -1, h.shape[-1])
            internal_mask = initial_mask[:, :, 1].unsqueeze(-1).expand(-1, -1, h.shape[-1])
